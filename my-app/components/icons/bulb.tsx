...
const AnimatedLightbulbFilled = ({ size = 20 }) => {
  ...
  useEffect(() => {
    Animated.loop(
      Animated.timing(anim, {
        toValue: 1,
        duration: 3000,
        useNativeDriver: false, // Necessário para interpolação de cores
      })
    ).start();
  }, []);

  const animatedFill = anim.interpolate({
...
export default AnimatedLightbulbFilled;
